...
class TestOptimizer(TestCase):
    ...
    def test_optimize_for_mobile_without_conv_bias(self):
        ...
        optimization_blocklist_no_fold_bn = {MobileOptimizerType.INSERT_FOLD_PREPACK_OPS}


        bn_fold_traced_module = optimize_for_mobile(bn_traced_module, optimization_blocklist_no_fold_bn)
        bn_input = torch.rand(1, 1, 6, 6)
        torch.testing.assert_close(bn_traced_module(bn_input), bn_fold_traced_module(bn_input), rtol=1e-2, atol=1e-3)
        ...
        bn3d_fold_traced_module = optimize_for_mobile(bn3d_traced_module)
        bn3d_input = torch.rand(1, 1, 6, 6, 6)
        torch.testing.assert_close(bn3d_traced_module(bn3d_input), bn3d_fold_traced_module(bn3d_input), rtol=1e-2, atol=1e-3)
